...
@WebMvcTest(RemindersOccurrencesEndpoint::class)
@AutoConfigureWebClient
@ActiveProfiles("test")
@ContextConfiguration(classes = [DefaultTestConfiguration::class])
class RemindersOccurrencesEndpointTest {
    @Autowired
    private lateinit var mockMvc: MockMvc

    @MockBean
    private lateinit var findUseCase: FindRemindersOccurrencesUseCase
    ...
    fun `acknowledge should return 404 status when trying to acknowledge a non-existing occurrence`() {
        whenever(acknowledgeUseCase.acknowledge(id = UUID.fromString("a2999215-db7d-49e9-91eb-15038e50182c")))
            .thenThrow(ReminderOccurrenceNotFoundException())
        mockMvc
            .perform(
                MockMvcRequestBuilders.put("/occurrences/a2999215-db7d-49e9-91eb-15038e50182c")
                    .contentType(MediaType.APPLICATION_JSON)
            ...
            .andExpect(
                MockMvcResultMatchers.jsonPath("$.errors[0].title")
                    .value(
                        Matchers.containsString("Reminder occurrence with such id is not found")
                    )
            )
    }
}
