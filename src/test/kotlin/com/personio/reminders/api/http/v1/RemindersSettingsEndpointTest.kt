...
@WebMvcTest(RemindersSettingsEndpoint::class)
@AutoConfigureWebClient
@ActiveProfiles("test")
@ContextConfiguration(classes = [DefaultTestConfiguration::class])
class RemindersSettingsEndpointTest {
    @Autowired
    ...
    @MockBean
    private lateinit var deleteUseCase: DeleteReminderUseCase
    ...
    @Test
    fun `delete should return 204 status when an existing reminder is deleted`() {
        doNothing().whenever(deleteUseCase).delete(UUID.fromString("a2999215-db7d-49e9-91eb-15038e50182c"))
        mockMvc
            .perform(
                MockMvcRequestBuilders.delete("/reminders/a2999215-db7d-49e9-91eb-15038e50182c")
                    .contentType(MediaType.APPLICATION_JSON)
                    .characterEncoding("utf-8")
            )
    ...
    @Test
    fun `delete should return 404 status when trying to remove a non-existing reminder`() {
        whenever(deleteUseCase.delete(reminderId = UUID.fromString("a2999215-db7d-49e9-91eb-15038e50182c")))
            .thenThrow(ReminderNotFoundException())
        mockMvc
            .perform(
                MockMvcRequestBuilders.delete("/reminders/a2999215-db7d-49e9-91eb-15038e50182c")
                    .contentType(MediaType.APPLICATION_JSON)
            ...
            .andExpect(
                MockMvcResultMatchers.jsonPath("$.errors[0].title")
                    .value(
                        Matchers.containsString("Reminder with such id is not found")
                    )
            )
    }
}
